...
def test_speed_baseload_calculations(ctg9):
    ...
    print(
        ...
        f'to run {len(cit)} cases of {ctg_input.ctg_operation_mode}-Load operation of {ctg9.name}')

    assert len(performance.mw) == len(cit)
    assert duration_in_seconds <= 0.5


def test_speed_2x1_minload_calculations(ctg9):
    cit = list(np.arange(-20, 120, 1))
    baro = [14.7] * len(cit)
    site_config = '2X1'
...
def test_speed_1x1_minload_calculations(ctg9):
    cit = list(np.arange(-20, 120, 1))
    baro = [14.7] * len(cit)
    site_config = '1X1'
    ctg_input = CtgInput(ctg_operation_mode=CtgOperationMode.MINLOAD.value,
...
def test_speed_70_percent_partload_calculations(ctg9):
    cit = list(np.arange(-20, 120, 1))
    baro = [14.7] * len(cit)
    site_config = '2X1'
    ctg_avlblty = 70
